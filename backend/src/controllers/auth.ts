...
import { generateRandomAvatarColor } from '../utils/randomAvatarColor';
import {
  loginRateLimit,
  registerRateLimit,
...
authRouter
  ...
  .post(
    ...
    async (c) => {
      ...
      await createIncome(safeUser.id);
  ...
  .post(
    ...
    describeRoute({
      ...
      responses: {
        200: response200(
          z.object({ message: z.literal('Password successfully reset') }),
        ),
    ...
    async (c) => {
      ...
      const tokens = await prisma.resetPassword.findMany({
        where: {
          expiredAt: {
            gt: new Date(),
          },
      ...
      const tokenMatch = await Promise.any(
        tokens.map(
          async (entry: {
            ...
            const isValid = await argon2.verify(entry.token, data.token);
            return isValid ? entry : Promise.reject();
          },
        ),
      ).catch(() => null);
      ...
      return c.json(
        { message: 'user.resetPassword.passwordResetSuccess' },
        200,
      );
    },
  )
...
async function createListCategories(userId: string) {
  ...
  await Promise.all(
    categories.map(async (category) => {
      ...
      if (!findColor) {
        throw new HTTPException(404, { message: 'Color not found' });
      }
