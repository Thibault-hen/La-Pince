...
async function main() {
	...
	const [
		categoryCoursesAlice,
		categoryTransportAlice,
		categoryLoisirsAlice,
		categorySanteAlice,
		categoryShoppingAlice,
	] = await prisma.$transaction([
		prisma.category.create({
			data: {
				title: 'category.food',
				userId: alice.id,
				colorId: colorMap['color.blue'].id,
			},
		}),
		prisma.category.create({
			data: {
				...
				userId: alice.id,
				colorId: colorMap['color.yellow'].id,
			},
		}),
		prisma.category.create({
			data: {
				...
				userId: alice.id,
				colorId: colorMap['color.green'].id,
			},
		}),
		prisma.category.create({
			data: {
				...
				userId: alice.id,
				colorId: colorMap['color.pink'].id,
			},
		}),
		prisma.category.create({
			data: {
				...
				userId: alice.id,
				colorId: colorMap['color.violet'].id,
			},
		}),
	]);
	// Categories for John
	const [
		categoryFacturesJohn,
		categoryAlimentationJohn,
		categoryVoitureJohn,
		categoryDivertissementJohn,
	] = await prisma.$transaction([
		prisma.category.create({
			data: {
				title: 'category.bills',
				userId: john.id,
				colorId: colorMap['color.red'].id,
			},
		}),
		prisma.category.create({
			data: {
		...
		prisma.category.create({
			data: {
				title: 'category.transport',
				userId: john.id,
				colorId: colorMap['color.gray'].id,
			},
		}),
		prisma.category.create({
			data: {
				title: 'category.entertainment',
				userId: john.id,
				colorId: colorMap['color.orange'].id,
			},
		}),
	]);
		...
		data: [
			{
				description: 'Super U',
				amount: 92.45,
				date: new Date(threeMonthsAgo.year, threeMonthsAgo.month - 1, 4),
				budgetId: budgetCoursesAlice1.id,
				userId: alice.id,
			},
			{
				description: 'Marché local',
				amount: 35.1,
				date: new Date(threeMonthsAgo.year, threeMonthsAgo.month - 1, 11),
				budgetId: budgetCoursesAlice1.id,
				userId: alice.id,
			},
			{
				description: 'Lidl',
				amount: 78.9,
				date: new Date(threeMonthsAgo.year, threeMonthsAgo.month - 1, 18),
				budgetId: budgetCoursesAlice1.id,
				userId: alice.id,
			},
			{
				...
				amount: 15.2,
				date: new Date(threeMonthsAgo.year, threeMonthsAgo.month - 1, 25),
				budgetId: budgetCoursesAlice1.id,
				userId: alice.id,
			},
		],
		...
		data: [
			{
				...
				amount: 75.2,
				date: new Date(twoMonthsAgo.year, twoMonthsAgo.month - 1, 1),
				budgetId: budgetTransportAlice2.id,
				userId: alice.id,
			},
			{
				description: 'Uber sortie',
				amount: 18.5,
				date: new Date(twoMonthsAgo.year, twoMonthsAgo.month - 1, 15),
				budgetId: budgetTransportAlice2.id,
				userId: alice.id,
			},
			{
				description: 'Cinéma',
				amount: 24.0,
				date: new Date(twoMonthsAgo.year, twoMonthsAgo.month - 1, 20),
				budgetId: budgetLoisirsAlice2.id,
				userId: alice.id,
			},
			{
				description: 'Restaurant entre amis',
				amount: 42.8,
				date: new Date(twoMonthsAgo.year, twoMonthsAgo.month - 1, 26),
				budgetId: budgetLoisirsAlice2.id,
				userId: alice.id,
			},
		],
		...
		data: [
			{
				...
				amount: 110.5,
				date: new Date(lastMonth.year, lastMonth.month - 1, 6),
				budgetId: budgetCoursesAlice3.id,
				userId: alice.id,
			},
			{
				description: 'Picard Surgelés',
				amount: 55.2,
				date: new Date(lastMonth.year, lastMonth.month - 1, 12),
				budgetId: budgetCoursesAlice3.id,
				userId: alice.id,
			},
			{
				...
				amount: 23.4,
				date: new Date(lastMonth.year, lastMonth.month - 1, 19),
				budgetId: budgetSanteAlice3.id,
				userId: alice.id,
			},
			{
				description: 'Rendez-vous médecin',
				amount: 25.0,
				date: new Date(lastMonth.year, lastMonth.month - 1, 28),
				budgetId: budgetSanteAlice3.id,
				userId: alice.id,
			},
		],
	...
	await prisma.expense.createMany({
		data: [
			...
			{
				description: 'Nouveaux vêtements',
				amount: 89.99,
				date: new Date(currentMonth.year, currentMonth.month - 1, 5),
				budgetId: budgetShoppingAliceCurrent.id,
				userId: alice.id,
			},
			{
				description: 'Sortie au bar',
				amount: 35.5,
				date: new Date(currentMonth.year, currentMonth.month - 1, 12),
				budgetId: budgetLoisirsAliceCurrent.id,
				userId: alice.id,
			},
			{
				description: 'Achat livre',
				amount: 22.0,
				date: new Date(currentMonth.year, currentMonth.month - 1, 15),
				budgetId: budgetLoisirsAliceCurrent.id,
				userId: alice.id,
			},
		],
		...
		data: [
			{
				description: 'Loyer',
				amount: 850.0,
				date: new Date(threeMonthsAgo.year, threeMonthsAgo.month - 1, 3),
				budgetId: budgetFacturesJohn1.id,
				userId: john.id,
			},
			{
				description: 'Facture électricité',
				amount: 75.5,
				date: new Date(threeMonthsAgo.year, threeMonthsAgo.month - 1, 14),
				budgetId: budgetFacturesJohn1.id,
				userId: john.id,
			},
			{
				...
				amount: 49.99,
				date: new Date(threeMonthsAgo.year, threeMonthsAgo.month - 1, 21),
				budgetId: budgetFacturesJohn1.id,
				userId: john.id,
			},
		],
		...
		data: [
			{
				...
				amount: 215.3,
				date: new Date(twoMonthsAgo.year, twoMonthsAgo.month - 1, 7),
				budgetId: budgetAlimentationJohn2.id,
				userId: john.id,
			},
			{
				description: 'Restaurant date',
				amount: 120.0,
				date: new Date(twoMonthsAgo.year, twoMonthsAgo.month - 1, 18),
				budgetId: budgetAlimentationJohn2.id,
				userId: john.id,
			},
			{
				...
				amount: 65.7,
				date: new Date(twoMonthsAgo.year, twoMonthsAgo.month - 1, 24),
				budgetId: budgetVoitureJohn2.id,
				userId: john.id,
			},
		],
	});
	...
	await prisma.expense.createMany({
		data: [
			// Dépenses du mois courant pour John
			{
				...
				amount: 130.45,
				date: new Date(currentMonth.year, currentMonth.month - 1, 3),
				budgetId: budgetAlimentationJohnCurrent.id,
				userId: john.id,
			},
			{
				description: "Plein d'essence",
				amount: 72.8,
				date: new Date(currentMonth.year, currentMonth.month - 1, 8),
				budgetId: budgetVoitureJohnCurrent.id,
				userId: john.id,
			},
			{
				description: 'Fast food midi',
				amount: 15.6,
				date: new Date(currentMonth.year, currentMonth.month - 1, 14),
				budgetId: budgetAlimentationJohnCurrent.id,
				userId: john.id,
			},
		],
		...
		data: [
			{
				...
				amount: 25.0,
				date: new Date(lastMonth.year, lastMonth.month - 1, 9),
				budgetId: budgetVoitureJohn3.id,
				userId: john.id,
			},
			{
				description: 'Révision annuelle',
				amount: 350.5,
				date: new Date(lastMonth.year, lastMonth.month - 1, 16),
				budgetId: budgetVoitureJohn3.id,
				userId: john.id,
			},
			{
				description: "Plein d'essence",
				amount: 70.1,
				date: new Date(lastMonth.year, lastMonth.month - 1, 23),
				budgetId: budgetVoitureJohn3.id,
				userId: john.id,
			},
			{
				description: 'Concert',
				amount: 89.0,
				date: new Date(lastMonth.year, lastMonth.month - 1, 29),
				budgetId: budgetDivertissementJohn3.id,
				userId: john.id,
			},
		],
	});
